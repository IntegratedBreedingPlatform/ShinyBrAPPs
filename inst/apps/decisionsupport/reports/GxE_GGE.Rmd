...
## Studies:
```{r}
knitr::kable(unique(data.table(rbindlist(rv_gxe$TD))[,.(trial,locationName,studyDbId,studyName)]))
```

## Trait: `r input$picker_trait`
## Analysis output
```{r, echo=FALSE}
TD.metangge <- rbindlist(rv_gxe$TD)[,.SD, .SDcols=c("trial","genotype",input$picker_trait)]
...
TDGGE <- tryCatch(gxeGGE(TD = rv_gxe$TD, trait = input$picker_trait, useWt = input$use_weights), error=function(e) e)
