    ...
    function(input, output, session){
      ...
      observeEvent(rv$data, {
        ...
        tryCatch({
              ...
              tryCatch({
                ...
                if (germ_resp$metadata$pagination$totalPages >1){
                  germplasm_data <- rbind(germplasm_data,rbindlist(lapply(2:germ_resp$metadata$pagination$totalPages, function(p){
                    handle_api_response(brapir::germplasm_attributevalues_get_search_searchResultsDbId(con = rv$con, searchResultsDbId = as.character(searchResultsDbId), page = p-1))$data
                  })))
                } 
                ...
                if (nrow(germplasm_data) > 0) {
                  ...
                  resp <- handle_api_response(brapir::germplasm_attributes_get_search_searchResultsDbId(con = rv$con, searchResultsDbId = as.character(searchResultsDbId)))
          ...
          showNotification(paste0("An error occured getting extradata: ", e$message), type = "error", duration = notification_duration)
        })
      })
    }
  )
